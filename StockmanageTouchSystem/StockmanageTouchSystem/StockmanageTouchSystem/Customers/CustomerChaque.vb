...
Public Class CustomerChaque
    ...
    Private Sub load_chaque()
        ...
        Dim adapter As New MySqlDataAdapter("SELECT `Chq_No`,`Customer_id`,`Name`,`Bank_id`,`Bank`,`Amount`,`Status`,`Issue_Date`,`Close_Date` FROM `customer_chaque` WHERE 1", MySqlConn)
        adapter.Fill(table)
    ...
    Private Sub loadcustomer()
        ...
        Dim bsource As New BindingSource
        Dim table As New DataTable
        Dim adapter As New MySqlDataAdapter("SELECT `id`,`Name`,`Address` FROM `customer` WHERE 1", MySqlConn)
        adapter.Fill(table)
        bsource.DataSource = table
    ...
    Private Sub C_NameTextBox_TextChanged(sender As Object, e As EventArgs) Handles C_NameTextBox.TextChanged
        ...
        MySqlConn.Close()
    End Sub
    Private Sub CustomerChaque_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub CustomerDataGridView_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles CustomerDataGridView.CellClick
        ...
        customerid = CustomerDataGridView.Rows(itm).Cells(0).Value
                ...
                Else
                    ...
                    Else
                        ...
                        Try
                            Dim Query As String
                            MySqlConn.Open()
                            Query = "INSERT INTO `customer_chaque` (`Chq_No`, `Customer_id`, `Name`, `Bank_id`, `Bank`, `Amount`, `Status`, `Issue_Date`, `Close_Date`) VALUES ('" & chqno & "', '" & customerid & "', '" & C_NameTextBox.Text & "', '1', '" & BankTextBox.Text & "', '" & AmountTextBox.Text & "', '" & State & "', '" & startDate & "', '" & CloseDate & "'); "
        ...
        If TextBox3.Text = "40012" Then
            ...
            If result = DialogResult.Yes Then
                Try
                    ...
                    Query = "DELETE FROM `customer_chaque` WHERE `customer_chaque`.`Chq_No` = '" & Chq_NoTextBox.Text & "'"
                    COMMAND = New MySqlCommand(Query, MySqlConn)
                    READER = COMMAND.ExecuteReader
                    MySqlConn.Close()
                    load_chaque()
        ...
        Else
            ...
            If result = DialogResult.Yes Then
                ...
                Try
                    Dim Query As String
                    MySqlConn.Open()
                    Query = "UPDATE `customer_chaque` SET `Status` = '" & StatusTextBox & "' where Chq_No='" & Chq_NoTextBox.Text & "'"
                    COMMAND = New MySqlCommand(Query, MySqlConn)
                    READER = COMMAND.ExecuteReader
                    MySqlConn.Close()
                    load_chaque()
    ...
    Private Sub Guna2TextBox6_TextChanged(sender As Object, e As EventArgs) Handles Guna2TextBox6.TextChanged
        If CheckBox1.Checked = False Then
            ...
            Dim bsource As New BindingSource
            Dim table As New DataTable()
            Dim adapter As New MySqlDataAdapter("SELECT `Chq_No`,`Customer_id`,`Name`,`Bank_id`,`Bank`,`Amount`,`Status`,`Issue_Date`,`Close_Date` FROM `customer_chaque` WHERE Close_Date BETWEEN '" & Stadate & "' And '" & enddat & "'", conn)
            adapter.Fill(table)
            bsource.DataSource = table
            ChaquereceivedDataGridView.DataSource = table
            Dim dv As New DataView(table)
        ...
        Else
            ...
            Dim bsource As New BindingSource
            Dim table As New DataTable()
            Dim adapter As New MySqlDataAdapter("SELECT `Chq_No`,`Customer_id`,`Name`,`Bank_id`,`Bank`,`Amount`,`Status`,`Issue_Date`,`Close_Date` FROM `customer_chaque`", conn)
            adapter.Fill(table)
            bsource.DataSource = table
            ChaquereceivedDataGridView.DataSource = table
            Dim dv As New DataView(table)
    ...
    Private Sub ChaquereceivedDataGridView_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles ChaquereceivedDataGridView.CellClick
        Dim k As Integer
        '`Chq_No`,`Customer_id`,`Name`,`Bank_id`,`Bank`,`Amount`,`Status`,`Issue_Date`,`Close_Date`'

        k = ChaquereceivedDataGridView.CurrentRow.Index
        Chq_NoTextBox.Text = ChaquereceivedDataGridView.Rows(k).Cells(0).Value.ToString
        customerid = ChaquereceivedDataGridView.Rows(k).Cells(1).Value 'Customer_id
    ...
    Private Sub getchaquefordate()
        If Guna2TextBox6.Text = "" Then
            ...
            Dim bsource As New BindingSource
            Dim table As New DataTable()
            Dim adapter As New MySqlDataAdapter("SELECT `Chq_No`,`Customer_id`,`Name`,`Bank_id`,`Bank`,`Amount`,`Status`,`Issue_Date`,`Close_Date` FROM `customer_chaque` WHERE Close_Date like '" & Stadate & "'", conn)
            adapter.Fill(table)
            bsource.DataSource = table
            ChaquereceivedDataGridView.DataSource = table
    ...
    Private Sub Guna2Button5_Click(sender As Object, e As EventArgs) Handles Guna2Button5.Click
        ...
        Dim chqrep As New CustomerChaqueReport
        chqrep.Refresh()
