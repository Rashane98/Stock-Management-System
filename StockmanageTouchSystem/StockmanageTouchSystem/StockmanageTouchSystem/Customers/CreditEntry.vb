...
Public Class CreditEntry
    ...
    Private Sub loaduserTbl() 'load user Table'
        ...
        Dim bsource As New BindingSource
        Dim table As New DataTable()
        Dim adapter As New MySqlDataAdapter("SELECT `id`,`Name`,`Address`,`TelNo`,`Fax`,`City`,`RegDate` FROM `customer` WHERE NOT `Name` = 'Cash'", conn)
        adapter.Fill(table)
        bsource.DataSource = table
        ...
        conn.Close()
    End Sub
    Private Sub CreditEntry_Load(sender As Object, e As EventArgs) Handles MyBase.Load
            ...
            If NameTxt.Text <> "" Then
                If AmountTxt.Text <> "" Then
                    ...
                    Try
                        ...
                        MySqlConn.Open()
                        Query = "INSERT INTO `customer_credit` (`id`, `Customer_id`, `Name`, `Amount`, `Credit_Date`) VALUES (NULL, '" & userid & "', '" & NameTxt.Text & "', '" & AmountTxt.Text & "', '" & duedate & "');"
    ...
    Private Sub EndDate_ValueChanged(sender As Object, e As EventArgs) Handles EndDate.ValueChanged
        ...
        Dim bsource As New BindingSource
        Dim table As New DataTable()
        Dim adapter As New MySqlDataAdapter("SELECT `id`,`Customer_id`,`Name`,`Amount`,`Credit_Date` FROM `customer_credit` WHERE `Credit_Date` BETWEEN '" & startdate & "' AND '" & enddate & "';", conn)
        adapter.Fill(table)
        bsource.DataSource = table
        CreditDetailsDataGridView1.DataSource = table
        conn.Close()
    ...
    Private Sub Guna2Button1_Click(sender As Object, e As EventArgs) Handles Guna2Button1.Click
        ...
        cryRpt.Refresh()
        cryRpt.RecordSelectionFormula = "{customer_credit1.Customer_id} = " & customer_id & ""
        cryRpt.Refresh()
    ...
    Private Sub CreditDetailsDataGridView1_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles CreditDetailsDataGridView1.CellContentClick
        ...
        customer_id = CreditDetailsDataGridView1.Rows(s).Cells(1).Value 'customer_id
        ...
        If PasswordTxt.Text = "40012" Then
            If customer_id <> 0 Then
                Try
                    Dim duedate As String = Format(Me.AddDateTimePicker.Value, "yyyy-MM-dd")
                    MySqlConn.Open()
                    Query = "UPDATE `customer_credit` SET `Amount` = '" & AmountTxt.Text & "', `Credit_Date` = '" & duedate & "' WHERE `customer_credit`.`id` = " & id
                    COMMAND = New MySqlCommand(Query, MySqlConn)
                    READER = COMMAND.ExecuteReader
                    MySqlConn.Close()
                    userid = 0
            ...
            If result = DialogResult.Yes Then
                If customer_id <> 0 Then
                    Try
                        ...
                        MySqlConn.Open()
                        Query = "DELETE FROM `customer_credit` WHERE `customer_credit`.`id` = " & id
                        COMMAND = New MySqlCommand(Query, MySqlConn)
                        READER = COMMAND.ExecuteReader
                        MySqlConn.Close()
                        userid = 0
