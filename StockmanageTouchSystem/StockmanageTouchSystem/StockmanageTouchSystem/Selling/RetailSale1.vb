...
Public Class RetailSale1
    ...
    Private Sub change_InvNo()
        ...
        Try
            Dim Query As String
            MySqlConn.Open()
            Query = "UPDATE `inv_no` SET `id` = '" & invno & "' WHERE `inv_no`.`id` = '" & InvLbl.Text & "';"
    ...
    Private Sub ShowRetailSale() 'load Retail Sale'
        ...
        Dim bsource As New BindingSource
        Dim table As New DataTable()
        Dim adapter As New MySqlDataAdapter("select barcode,des,qty,unit_price,cus_price,discount,expire_date,amount,id,Invno,billing_date,Customer_id,CusName,itemid from selling_temp_1", conn)
        adapter.Fill(table)
        bsource.DataSource = table
        ...
        conn.Close()
    End Sub
    Private Sub RetailSale1_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub AddtoGrideView()
        ...
        If isin = 1 Then
            ...
            Try
                Dim Query As String
                MySqlConn.Open()
                Query = "UPDATE `selling_temp_1` SET `qty` = '" & nqty & "', `unit_price` = '" & Unit_PriceTextBox.Text & "', `discount` = '" & discountno & "', `amount` = '" & namt & "' WHERE `selling_temp_1`.`itemid` = '" & itemid & "'; "
                COMMAND = New MySqlCommand(Query, MySqlConn)
                READER = COMMAND.ExecuteReader
                MySqlConn.Close()
        ...
        Else
            Try
                Dim Query As String
                MySqlConn.Open()
                Query = "INSERT INTO `selling_temp_1` (`id`, `Invno`, `billing_date`, `Customer_id`, `CusName`, `itemid`, `barcode`, `des`, `qty`, `unit_price`, `cus_price`, `discount`, `amount`, `expire_date`) VALUES (NULL, '" & InvLbl.Text & "', '" & billdate & "', '" & customerid & "', '" & CNameTxt.Text & "', '" & itemid & "', '" & BarCodeTxt.Text & "', '" & DesTxt.Text & "', '" & QutTextBox.Text & "', '" & Unit_PriceTextBox.Text & "', '" & CmPrice & "', '" & discountno & "', '" & AmountTextBox.Text & "', '" & expireDate & "');"
                COMMAND = New MySqlCommand(Query, MySqlConn)
                READER = COMMAND.ExecuteReader
                MySqlConn.Close()
    ...
    Private Function addtothrselling()
        ...
        Try
            MySqlConn.Open()
            Query = "DELETE FROM `selling_temp_1`"
            COMMAND = New MySqlCommand(Query, MySqlConn)
            READER = COMMAND.ExecuteReader
            MySqlConn.Close()
        Catch ex As Exception
    ...
    Private Sub DeleteBtn_Click(sender As Object, e As EventArgs) Handles DeleteBtn.Click
        ...
        If result = DialogResult.Yes Then
            ...
            If Del_check_isblane_product(newbar) = True Then
                ...
                Try
                    MySqlConn.Open()
                    Query = "DELETE FROM `selling_temp_1` WHERE `selling_temp_1`.`id` = '" & invid & "'"
                    COMMAND = New MySqlCommand(Query, MySqlConn)
                    READER = COMMAND.ExecuteReader
                    MySqlConn.Close()
                Catch ex As Exception
