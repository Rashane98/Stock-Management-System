...
Public Class RetailSale2
    ...
    Private Sub ShowRetailSale() 'load Retail Sale'
        ...
        conn.Close()
    End Sub
    Private Sub RetailSale2_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub DesTxt_TextChanged(sender As Object, e As EventArgs) Handles DesTxt.TextChanged
        If BarCodeTxt.Text = "" Then
            ...
            Dim bsource As New BindingSource
            Dim table As New DataTable()
            Dim adapter As New MySqlDataAdapter("SELECT barcode,Des,cost,whprice,rprice,stock,CMshowprice,ExpireDate,id FROM all_items_view Where NOT stock = 0 order by barcode;", conn)
            adapter.Fill(table)
            bsource.DataSource = table
            ItemsShow.DataSource = table
        ...
        Else
            ...
            Dim bsource As New BindingSource
            Dim table As New DataTable()
            Dim adapter As New MySqlDataAdapter("SELECT barcode,Des,cost,whprice,rprice,stock,CMshowprice,ExpireDate,id FROM all_items_view where barcode like '%" & BarCodeTxt.Text & "%' AND NOT stock = 0  order by barcode;", conn)
            adapter.Fill(table)
            bsource.DataSource = table
            ItemsShow.DataSource = table
            Dim dv As New DataView(table)
    ...
    Private Function changestock() 'This function for the change stock manage'
        ...
        Dim nstock As Integer = stock - nqty
